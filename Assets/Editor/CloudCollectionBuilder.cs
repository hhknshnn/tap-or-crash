...
internal static class CloudCollectionBuilder
{
    const string ModelDir = "Assets/Models/CloudPlanets";
    const string SpriteDir = "Assets/Sprites/CloudPlanets";
    const string Prefab3DDir = "Assets/Prefabs/Cloud3D";
    const string MaterialDir = ModelDir + "/Materials";

    [MenuItem("Tools/Tap or Crash/Build Cloud Collection")]
    static void Build()
    {
        EnsureFolder("Assets/Prefabs", "Cloud3D");
        EnsureFolder(ModelDir, "Materials");

        Material cloudMaterial = BuildMaterial();
        GameObject[] gameplayPrefabs = new GameObject[10];

        for (int i = 1; i <= 10; i++)
        {
            string name = $"Cloud_{i:00}";
            ConfigureModelImporter(name);
            gameplayPrefabs[i - 1] = BuildSpritePrefab(name);
            Build3DPrefab(name, cloudMaterial);
        }

        AddOrUpdateLevel(gameplayPrefabs);
        AssetDatabase.SaveAssets();
        AssetDatabase.Refresh();
        Debug.Log("Cloud collection integration complete: 10 gameplay prefabs, 10 3D prefabs.");
    }
    ...
    static void ConfigureModelImporter(string name)
    {
        ...
        if (importer == null)
            throw new InvalidOperationException("Missing Cloud model importer: " + path);
    ...
    {
        string path = MaterialDir + "/Cloud_Palette_URP.mat";
        ...
        if (material == null)
        {
            ...
            material = new Material(shader) { name = "Cloud_Palette_URP" };
            AssetDatabase.CreateAsset(material, path);
        }

        // Vapour: fully rough, no metal, so the white body keeps its form.
        material.SetFloat("_Smoothness", 0.06f);
        material.SetFloat("_Metallic", 0f);

        Texture2D palette = AssetDatabase.LoadAssetAtPath<Texture2D>(
            ModelDir + "/Cloud_Palette.png");
        if (palette != null)
        {
    ...
    static GameObject BuildSpritePrefab(string name)
    {
        ...
        if (importer == null)
            throw new InvalidOperationException("Missing Cloud sprite: " + pngPath);
        ...
        if (sprite == null)
            throw new InvalidOperationException("Cloud sprite import failed: " + pngPath);

        GameObject root = new GameObject(name);
        SpriteRenderer renderer = root.AddComponent<SpriteRenderer>();
        renderer.sprite = sprite;
        root.AddComponent<CloudPlanetAmbience>();
    ...
    static void Build3DPrefab(string name, Material material)
    {
        ...
        if (model == null)
            throw new InvalidOperationException("Missing Cloud model: " + modelPath);
    ...
    static void AddOrUpdateLevel(GameObject[] cloudPrefabs)
    {
        ...
        PlanetSpawner.PlanetLevel cloud = null;
        for (int i = 0; i < levels.Count; i++)
        {
            if (levels[i] != null
                && string.Equals(levels[i].levelName, "Cloud", StringComparison.OrdinalIgnoreCase))
            {
                cloud = levels[i];
                break;
            }
        ...
        if (cloud == null)
        {
            cloud = new PlanetSpawner.PlanetLevel();
            levels.Add(cloud);
        }

        cloud.levelName = "Cloud";
        cloud.prefabs = cloudPrefabs;
