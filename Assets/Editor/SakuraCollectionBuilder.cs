...
internal static class SakuraCollectionBuilder
{
    const string ModelDir = "Assets/Models/SakuraPlanets";
    const string SpriteDir = "Assets/Sprites/SakuraPlanets";
    const string Prefab3DDir = "Assets/Prefabs/Sakura3D";
    const string MaterialDir = ModelDir + "/Materials";

    [MenuItem("Tools/Tap or Crash/Build Sakura Collection")]
    static void Build()
    {
        EnsureFolder("Assets/Prefabs", "Sakura3D");
        EnsureFolder(ModelDir, "Materials");

        Material sakuraMaterial = BuildMaterial();
        GameObject[] gameplayPrefabs = new GameObject[10];

        for (int i = 1; i <= 10; i++)
        {
            string name = $"Sakura_{i:00}";
            ConfigureModelImporter(name);
            gameplayPrefabs[i - 1] = BuildSpritePrefab(name);
            Build3DPrefab(name, sakuraMaterial);
        }

        AddOrUpdateLevel(gameplayPrefabs);
        AssetDatabase.SaveAssets();
        AssetDatabase.Refresh();
        Debug.Log("Sakura collection integration complete: 10 gameplay prefabs, 10 3D prefabs.");
    }
    ...
    static void ConfigureModelImporter(string name)
    {
        ...
        if (importer == null)
            throw new InvalidOperationException("Missing Sakura model importer: " + path);
    ...
    {
        string path = MaterialDir + "/Sakura_Palette_URP.mat";
        ...
        if (material == null)
        {
            ...
            material = new Material(shader) { name = "Sakura_Palette_URP" };
            AssetDatabase.CreateAsset(material, path);
        }

        // Matte petals and bark: the opposite of the crystal pack's glossy coat.
        material.SetFloat("_Smoothness", 0.16f);
        material.SetFloat("_Metallic", 0f);

        Texture2D palette = AssetDatabase.LoadAssetAtPath<Texture2D>(
            ModelDir + "/Sakura_Palette.png");
        if (palette != null)
        {
    ...
    static GameObject BuildSpritePrefab(string name)
    {
        ...
        if (importer == null)
            throw new InvalidOperationException("Missing Sakura sprite: " + pngPath);
        ...
        if (sprite == null)
            throw new InvalidOperationException("Sakura sprite import failed: " + pngPath);

        GameObject root = new GameObject(name);
        SpriteRenderer renderer = root.AddComponent<SpriteRenderer>();
        renderer.sprite = sprite;
        root.AddComponent<SakuraPlanetAmbience>();
    ...
    static void Build3DPrefab(string name, Material material)
    {
        ...
        if (model == null)
            throw new InvalidOperationException("Missing Sakura model: " + modelPath);
    ...
    static void AddOrUpdateLevel(GameObject[] sakuraPrefabs)
    {
        ...
        PlanetSpawner.PlanetLevel sakura = null;
        for (int i = 0; i < levels.Count; i++)
        {
            if (levels[i] != null
                && string.Equals(levels[i].levelName, "Sakura", StringComparison.OrdinalIgnoreCase))
            {
                sakura = levels[i];
                break;
            }
        ...
        if (sakura == null)
        {
            sakura = new PlanetSpawner.PlanetLevel();
            levels.Add(sakura);
        }

        sakura.levelName = "Sakura";
        sakura.prefabs = sakuraPrefabs;
